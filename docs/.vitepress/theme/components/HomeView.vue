...
const runTypeIt = () => {
  new TypeIt('#type-it', {
    ...
    loop: true, // 无限循环
    loopDelay: 2000, // 循环间隔 2 秒
    deleteSpeed: 40, // 删除速度
    breakLines: false, // 不换行，删除后再打下一段
  }).go()
}
...
const imgSrc = computed(() => {
  ...
  // 图片位于 `docs/.vitepress/theme/assets/images/indexBg.png`
  // 使用 Vite 打包时的模块 URL，这样文件会被正确处理并返回打包后真实 URL
  return new URL('../assets/images/indexBg.png', import.meta.url).href
})
...
<template>
  <div class="home-view">
    <div class="h-screen relative overflow-hidden">
      ...
      <video
        v-if="!isDark"
        class="video-bg min-h-screen w-full object-cover"
        autoplay
        loop
        muted
        playsinline
      >
        <source src="/wallpaper-light.mp4" type="video/mp4" />
      </video>
      <!-- 暗黑模式：视频背景 -->
      <video
        v-else
        ...
        playsinline
      >
        <source src="/wallpaper-dark.mp4" type="video/mp4" />
      </video>
      <p
        id="type-it"
        class="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] text-xl md:text-4xl font-bold duration-1000 text-white"
      ></p>
    </div>
