...
const getContent = async (pathWay) => {
  try {
    ...
    // 尝试解析 YAML，如果失败则返回 false
    let yamlObj
    try {
      yamlObj = yaml.load(yamlArr[1])
    } catch (yamlError) {
      console.log(chalk.yellow(`⚠️  YAML 解析失败: ${pathWay}`))
      return false
    }
    
    if (!yamlObj || (!yamlObj.date && !yamlObj.categories && !yamlObj.tags)) return false
    ...
    console.log(chalk.yellow(`⚠️  读取文件失败: ${pathWay}`))
    return false
  }
}
